...
Class User_model extends CI_Model
{
 function login($username, $password)
 {
   ...
   if (strpos($username, '@') !== true) {
    $this->db->where('savsoft_users.ADMISSION_NUMBER', $username);
   }else{
...
$this->db->limit(1);
 ...
 function resend($email){
  $this -> db -> where('savsoft_users.ADMISSION_NUMBER', $email);
   // $this -> db -> where('savsoft_users.verify_code', '0');
    $this -> db -> join('savsoft_group', 'savsoft_users.gid=savsoft_group.gid');
  $this->db->limit(1);
    $query = $this -> db -> get('savsoft_users');
...
return $revenue;
 ...
 function user_list($limit){
	 if($this->input->post('search')){
		 ...
		 $this->db->or_where('savsoft_users.ADMISSION_NUMBER',$search);
 ...
 function insert_user(){
	  $logged_in=$this->session->userdata('logged_in');
		...
		'ADMISSION_NUMBER'=>$this->input->post('regno'),
